...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if num_classes == 80 or dataset == 'coco' or dataset =='robocup':
      self.names = coco_class_name

    elif dataset == 'line_4_cat':
      self.names = ['horizontal', 'vertical', 'top_left', 'top_right']
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    cat = int(cat)
    print()
    print('cat', cat, "self.names:",self.names)
    print()
  ...
  def add_lines(self, bbox, cat, conf=1,show_txt=False, img_id='default'):
    bbox = np.array(bbox, dtype=np.int32)
